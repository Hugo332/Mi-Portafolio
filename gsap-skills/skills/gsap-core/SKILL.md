...
- **easeReverse** _(GSAP 3.15+)_ — Controls the easing applied when a tween plays **backwards** (`.reverse()`, `yoyo` second half, or any negative-direction playback). Default behavior reverses the curve too, so `power3.out` becomes `power3.in` on the way back, which often feels wrong for UI toggles. Set `easeReverse: true` to **reapply the forward ease** in the reverse direction, or pass an ease string/function for **completely different** reverse easing. Replaces the deprecated `yoyoEase` (kept for backwards compatibility). Cascades through timeline `defaults`. Recalculates from the current point if direction changes mid-animation.

  ```javascript
  // Modal that opens with overshoot and closes with the same overshoot feel
  gsap.to('.modal', { y: 0, autoAlpha: 1, duration: 0.4, ease: 'back.out(1.6)', easeReverse: true });

  // Different ease per direction
  gsap.to('.drawer', { x: 0, ease: 'power3.out', easeReverse: 'power2.in' });
  ```

  Use for: toggleable UI (modals, drawers, menus), animations that get interrupted/reversed mid-flight, and any `.reverse()` scenario where the default flipped curve looks wrong.
...
- **CSS variables** — GSAP can animate **custom properties as targets** (e.g. `"--hue": 180`, `"--size": 100`) and, since **GSAP 3.13+**, animate **to a `var(--token)` value** so design tokens stay the source of truth instead of hardcoding hex/rgb in JS:

  ```javascript
  // Animate to a CSS variable value (3.13+)
  gsap.to('.btn', { backgroundColor: 'var(--accent)', duration: 0.3 });

  // Flash a border via tokens — no need to repeat #d83a52 / #e3e7ef in JS
  gsap.fromTo(input,
    { borderColor: 'var(--error)' },
    { borderColor: 'var(--border)', duration: 1.2, ease: 'power2.out' }
  );
  ```

  The variable is resolved at tween-render time against the element’s computed styles, so theming and runtime overrides (e.g. dark mode toggling `:root` vars) keep working.
